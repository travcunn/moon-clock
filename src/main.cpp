...
void displayInfo()
{
  ...
  dateBuffer = String(gps.date.month()) + "/" + String(gps.date.day()) + "/" + String(gps.date.year());

  uint8_t hour = (gps.time.hour() + int(decodeTimezone(gps.location.lat(), gps.location.lng())) + 24) % 24;
  ...
  {
    weekdayBuffer = daysOfWeek[timeinfo.tm_wday];
  }
...
void loop()
{
  ...
  {
    uint8_t hour = (gps.time.hour() + int(decodeTimezone(gps.location.lat(), gps.location.lng())) + 24) % 24;
    setSystemTime(gps.date.year(), gps.date.month(), gps.date.day(), hour, gps.time.minute(), gps.time.second());
    displayInfo();
    display.hibernate();
    delay(180000);
  }
